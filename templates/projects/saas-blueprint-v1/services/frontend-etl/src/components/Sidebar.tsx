...
import { House, User, SignOut, CaretRight, CaretLeft, Sun, Moon, ArrowSquareOut, Lightning } from '@phosphor-icons/react'
...
export default function Sidebar() {
  ...
  // Link de retorno ao frontend principal (href estático — não precisa de OTT)
  const mainBase = window.location.port === '3345' ? 'http://localhost:5178' : 'http://localhost:5177'
  const [mainHov, setMainHov] = useState(false)

  // Logout: invalida sessão no backend e ordena ao frontend principal que também
  // limpe sua sessão via ?force_logout=1 — sem delay, sem acesso residual.
  function handleLogout() {
    logout()
    window.location.href = `${mainBase}?force_logout=1`
  }
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        {/* Frontend Principal: <a> estático — clique normal (mesma aba) ou clique-direito → nova aba */}
        <div style={{ position: 'relative' }}>
          <a
            href={mainBase}
            ...
            style={{
              ...
              background: mainHov ? overlay : 'transparent',
              color: content, textDecoration: 'none',
              fontSize: 13, fontWeight: 500, transition: 'background .15s',
            }}
          >
            <ArrowSquareOut size={17} weight="regular" style={{ flexShrink: 0, color: muted, transform: 'scaleX(-1)' }} />
            {!collapsed && <span style={{ flex: 1, textAlign: 'left' }}>Frontend Principal</span>}
          </a>
          <Tooltip label="Frontend Principal" visible={collapsed && mainHov} />
        </div>
