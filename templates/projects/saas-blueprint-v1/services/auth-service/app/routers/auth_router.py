...
from app.schemas.auth_schemas import (
    LoginRequest, LoginResponse, RefreshRequest, RefreshResponse, UserPayload
)
...
@router.post("/logout")
def logout(body: RefreshRequest, db: Session = Depends(get_db_session)):
    """
    ...
    row = db.execute(
        ...
        {"rth": hash_refresh_token(body.refresh_token)},
    ).fetchone()
    if row:
