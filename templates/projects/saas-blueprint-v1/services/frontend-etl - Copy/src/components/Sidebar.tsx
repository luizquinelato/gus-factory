/**
 * Sidebar — ETL Frontend
 * Cópia fiel do Sidebar do frontend principal.
 * Diferenças: só "Home" no nav, sem Configurações/admin, logout redireciona ao frontend principal.
 */
import React, { useState, useRef, useEffect, useCallback, useMemo } from 'react'
import { createPortal } from 'react-dom'
import { NavLink } from 'react-router-dom'
import { House, SignOut, CaretLeft, CaretRight, Sun, Moon, User } from '@phosphor-icons/react'
import { useAuth } from '../contexts/AuthContext'
import { useTheme } from '../contexts/useTheme'
import apiClient from '../services/apiClient'

/* ── Design System Tokens — idênticos ao frontend principal ───────────────── */
const SB = {
  surface: { light: '#F8FAFC', dark: '#1E2233' },
  header:  { light: '#E2E8F0', dark: '#161929' },
  content: { light: '#1E293B', dark: '#F1F5F9' },
  muted:   { light: '#475569', dark: '#94A3B8' },
  overlay: { light: '#E2E8F0', dark: '#252B42' },
} as const
...
interface FlyoutProps {
  ...
  isDark?: boolean; alignBottom?: boolean
}
function Flyout({ anchorRef, open, onClose, children, isDark, alignBottom }: FlyoutProps) {
  ...
  return createPortal(
    <div ref={ref} style={{
      ...
      background: isDark ? '#1C2035' : '#ffffff', borderRadius: 12,
      boxShadow: isDark ? '0 8px 32px rgba(0,0,0,.50)' : '0 8px 32px rgba(0,0,0,.18)',
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 16px',
        background: hovBg, border: 'none', cursor: 'pointer', textAlign: 'left',
        color, fontSize: 14, fontWeight: 500, transition: 'background .12s' }}>
...
function Tooltip({ label, visible }: { label: string; visible: boolean }) {
  ...
  return (
    <div style={{
      ...
      background: '#1e293b', color: '#f8fafc', fontSize: 12, fontWeight: 600,
      ...
      <span style={{ position: 'absolute', right: '100%', top: '50%', transform: 'translateY(-50%)',
        border: '4px solid transparent', borderRight: '5px solid #1e293b' }} />
    </div>
  )
}
...
interface FooterBtnProps {
  icon: React.ElementType; label: string; collapsed: boolean; onClick: () => void
  content: string; overlay: string; iconColor?: string; weight?: 'regular' | 'fill'
}
const FooterBtn = React.forwardRef<HTMLButtonElement, FooterBtnProps>(
  function FooterBtn({ icon: Icon, label, collapsed, onClick, content, overlay, iconColor, weight = 'regular' }, ref) {
    const [hov, setHov] = useState(false)
    return (
      <div style={{ position: 'relative' }}>
        <button ref={ref} onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
          style={{ display: 'flex', alignItems: 'center', gap: 10,
            padding: collapsed ? '8px 0' : '8px 10px',
            justifyContent: collapsed ? 'center' : 'flex-start',
            width: '100%', border: 'none', cursor: 'pointer', borderRadius: 8,
            background: hov ? overlay : 'transparent', color: content,
            fontSize: 13, fontWeight: 500, transition: 'background .15s' }}>
          <Icon size={17} weight={weight} style={{ flexShrink: 0, color: iconColor ?? content }} />
...
function NavItem({ to, icon: Icon, label, collapsed, selBg, selColor, content, overlay }: NavItemProps) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <NavLink to={to} end onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        style={({ isActive }) => ({
...
export default function Sidebar() {
  ...
  // Avatar: gradiente color1→color2 com on_gradient_1_2 foreground (igual ao frontend principal)
  ...
  // Logout: limpa sessão local e redireciona ao frontend principal
  function handleLogout() {
    logout()
    const main = window.location.port === '3345' ? 'http://localhost:5178' : 'http://localhost:5177'
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <img src="/favicon.svg" alt="logo" style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0 }} />
        {!collapsed && (
      ...
      <div style={{ position: 'absolute', top: '50%', right: -13, transform: 'translateY(-50%)', zIndex: 1 }}>
        <button onClick={() => setCollapsed(v => !v)} title={collapsed ? 'Expandir' : 'Recolher'}
          style={{ width: 26, height: 26, borderRadius: '50%', background: circleBg, border: `1px solid ${circleB}`,
      ...
      {/* ── FOOTER ── */}
      <div style={{ padding: collapsed ? '10px 0' : '10px 10px 14px', borderTop: `1px solid ${border}`, display: 'flex', flexDirection: 'column', gap: 4 }}>

        {/* Theme toggle — segmented (expandido) / ícone (colapsado) */}
        {collapsed ? (
          <FooterBtn icon={isDark ? Moon : Sun} label={isDark ? 'Dark Mode' : 'Light Mode'}
            collapsed={collapsed} onClick={toggleTheme} content={content} overlay={overlay} iconColor={muted} />
        ) : (
          <div style={{ display: 'flex', borderRadius: 10, padding: 3, background: isDark ? 'rgba(0,0,0,0.30)' : 'rgba(0,0,0,0.07)', margin: '2px 0' }}>
            {([
              ...
              <button key={label} onClick={onClick} style={{
                ...
                color: active ? content : muted, fontWeight: active ? 600 : 500, fontSize: 12,
                boxShadow: active ? '0 1px 4px rgba(0,0,0,.14)' : 'none', transition: 'all .2s',
              }}>
        ...
        <div style={{ position: 'relative' }}>
          <button ref={profileBtnRef}
            ...
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: avatarBg,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: avatarFg, fontWeight: 700, fontSize: 14, flexShrink: 0, overflow: 'hidden' }}>
              {user?.avatar_url
        ...
        <Flyout anchorRef={profileBtnRef} open={profileOpen} onClose={() => setProfileOpen(false)} isDark={isDark} alignBottom>
          ...
          <div style={{ padding: '4px 0' }}>
            <FlyoutItem icon={User}    label="Meu Perfil"    isDark={isDark} onClick={() => setProfileOpen(false)} />
            <FlyoutItem icon={SignOut} label="Sair da conta" isDark={isDark} danger onClick={() => { setProfileOpen(false); handleLogout() }} />
          </div>
        </Flyout>
